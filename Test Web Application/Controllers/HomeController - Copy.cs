    ...
    public class HomeController : Controller
    {
        ...
        public ActionResult GetEmployeeSlot(int empId, string dt)
        {
            ...
            foreach (var empSlot in empSlotL)
            {
                ...
                string fromTime = storedTime.TotalSeconds.ToString();// fromTime=new DateTime().Add(storedTime).ToString("h:mm tt").ToLower();
                // string fromTime = empSlot.MeetingFromTime?.ToString("hh:mm tt");
                storedTime = (TimeSpan)empSlot.MeetingToTime;
                string ToTime = (storedTime.TotalSeconds - 1).ToString();//ToTime = new DateTime().Add(storedTime).ToString("h:mm tt").ToLower();// empSlot.MeetingToTime?.ToString("h:mmtt");
